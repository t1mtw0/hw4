...
/**
 * @brief Returns true if all paths from leaves to root are the same length
 * (height), and false otherwise
 *
 *        Note: this doesn't mean we are checking if the tree is full, but just
 * that any leaf node (wherever it may exist) has the same length path to the
 * root as all others.
 *
